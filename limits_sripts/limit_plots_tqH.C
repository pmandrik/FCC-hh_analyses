...
class BrasilDrawer_fix{
    ...
    TCanvas * draw_brasil_plot(vector<double> * x_axis, vector<double> * sigma_2_down, vector<double> * sigma_1_down, vector<double> * central, vector<double> * sigma_1_up, vector<double> * sigma_2_up, vector<double> * central_median){
      ...
      leg->AddEntry(gr_central, "Br(t #rightarrow u H ) 95% CL Expected Limit", "l");
      leg->AddEntry(gr_1_sigma, "Br(t #rightarrow u H ) #pm 1 std. deviation", "f");
      leg->AddEntry(gr_2_sigma, "Br(t #rightarrow u H ) #pm 2 std. deviation", "f");
      leg->AddEntry(gr_central_alt, "Br(t #rightarrow c H ) 95% CL Expected Limit", "l");
      leg->Draw();
...
double get_branch(double limit){
  ...
  double xsec_fcnc = 1.0 * limit * 0.01; 
  double xsec_sm = 213.5;
  // xsec_fcnc = 2 * xsec_sm * Br(fcnc) * Br(h->bb) * Br(SM)
  // Br(fcnc) = xsec_fcnc / (2 * xsec_sm * Br(h->bb) * Br(SM))
  double branch = xsec_fcnc / (2 * xsec_sm * 0.5824);
  
  return branch;
}

double get_kappa(double br_limit){
  double kappa = TMath::Sqrt(br_limit / 0.0388);
  return kappa;
}

void limit_plots_tqH(){
  string mode = "u";
  ...
  for(int lumi : lumis){
    string name = "combo_wd/weight_tune_5_cut_BDT_2_000_25_" + to_string(lumi) + "_CUT.root";
           ...
           name = "tqH/weight_BDT_0c_000_25_" + to_string(lumi) + "_tau.root";
    string alt_name = "combo_wd/weight_tune_5_tac2_BDT_1_000_20_" + to_string(lumi) + "_CUT_tac.root";
           ...
           alt_name = "tqH/weight_BDT_0c_000_25_" + to_string(lumi) + "_tac.root";
